...
namespace database{
    ...
    size_t Database::get_numbers_of_shard()
    {
        return 2;
    }
    ...
    std::vector<std::string> Database::get_all_hints()
    {
        ...
        for(size_t i=0;i<=get_numbers_of_shard();++i)
        {
    ...
    std::string Database::sharding_hint_for_user(long id)
    {
        size_t shard_number = std::hash<std::string>{}(std::to_string(id)) % get_numbers_of_shard();
    ...
    Poco::Data::Transaction Database::create_transaction(Poco::Data::Session& session) 
    {
        return Poco::Data::Transaction(session, false);
    }
}